...
import { getPillowById } from 'apiSdk/pillows';
...
function PillowViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PillowInterface>(
    ...
    () =>
      getPillowById(id, {
        relations: ['organization', 'inventory'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('pillow', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/pillows/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('inventory', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/inventories/create?pillow_id=${data?.id}`}>
